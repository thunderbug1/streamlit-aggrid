...
def AgGrid(
    ...
    **default_column_parameters:
    ...
    #         else:
    #             return value.__str__()
    #     row_data = df.to_json(orient = "records", default_handler=cast_to_serializable, date_format="iso")
        
    #     # json_frame = df.applymap(cast_to_serializable)
    #     # row_data2 = df.to_json(orient = "records", default_handler=cast_to_serializable, date_format="iso")
    #     # row_data = json_frame.to_dict(orient="records")
    #     # row_data = simplejson.dumps(row_data, ignore_nan=True)
    #     # with open("cur_row_data.json", "w") as f:
    #     #     f.write(row_data)
    #     # with open("row_data2.json", "w") as f:
    #     #     f.write(row_data2)
    #     return row_data
